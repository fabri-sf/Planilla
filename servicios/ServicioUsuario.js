...
class ServicioUsuario {
    ...
    async Create(datos) {
        ...
        const token = jwt.sign({ rol: datos.rol, correo: datos.correo }, this.PalabraSecreta, { expiresIn: '10m' });
    ...
    async Autenticacion(correo, ClaveSinEncriptar) {
        ...
        const usuarios = await ejecutarConsulta("SELECT * FROM USUARIO WHERE correo = ? AND activo = TRUE", [correo]);
    ...
    async GenerarToken(rol, correo) {
        let token = jwt.sign({ rol, correo }, this.PalabraSecreta, { expiresIn: '10m' });
        // Almacenar en la base de datos para el usuario
        await ejecutarConsulta("UPDATE USUARIO SET token = ? WHERE correo = ?", [token, correo]);
    ...
    async ValidarToken(solicitud) {
        ...
        const usuarios = await ejecutarConsulta("SELECT * FROM USUARIO WHERE correo = ?", [Resultado.correo]);
        if (!usuarios || usuarios.length === 0) return false;
        const Usuario = usuarios[0];
    ...
    async DesAutenticacion(correo) {
        ...
        await ejecutarConsulta("UPDATE USUARIO SET token = NULL WHERE correo = ?", [correo]);
    }
}
